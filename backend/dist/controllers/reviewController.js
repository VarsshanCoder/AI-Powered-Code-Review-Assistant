...
class ReviewController {
    ...
    async getReview(req, res) {
        try {
            ...
            logger_1.logger.error('Failed to get review:', error);
    ...
    async getReviews(req, res) {
        try {
            ...
            logger_1.logger.error('Failed to get reviews:', error);
    ...
    async addComment(req, res) {
        try {
            ...
            index_1.io.to(`review-${id}`).emit('new-comment', comment);
    ...
    async applyAutoFix(req, res) {
        try {
            ...
            logger_1.logger.error('Failed to apply auto-fix:', error);
            res.status(500).json({ error: 'Failed to apply auto-fix' });
        }
    }
    async analyzeCodeInBackground(reviewId, repository, commitSha, prNumber) {
        try {
            logger_1.logger.info(`Starting AI analysis for review ${reviewId}`);
            const [owner, repo] = repository.fullName.split('/');
            let filesToAnalyze = [];
            if (prNumber) {
                // Analyze PR files
            ...
            for (const file of filesToAnalyze) {
                if (file.status === 'removed')
                    continue;
                try {
                    const content = await githubService_1.githubService.getFileContent('', owner, repo, file.filename, commitSha);
                    const language = this.detectLanguage(file.filename);
                    if (!language)
                        continue;
                    const analysis = await aiService_1.aiService.analyzeCode(content, language, file.filename);
                    ...
                    logger_1.logger.error(`Failed to analyze file ${file.filename}:`, error);
                }
            }
            ...
            // Emit real-time update
            index_1.io.to(`review-${reviewId}`).emit('analysis-complete', {
                reviewId,
                score: finalScore,
                findingsCount: findings.length
            });
            logger_1.logger.info(`Completed AI analysis for review ${reviewId}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to analyze review ${reviewId}:`, error);
            await prisma.review.update({
                where: { id: reviewId },
                data: { status: 'FAILED' }
            });
